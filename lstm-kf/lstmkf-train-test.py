...
- Dataset: NGSIM processed CSV, 每行一个样本；字段 x_hist_flat / y_fut_flat
  每帧为 (x,y,mask)*7 或 (x,y)*7。
- Logging: TensorBoard (train/loss, val/loss)。
- Visualization: 训练(或评估)后，画 1 张可视化图（轨迹风格与你脚本一致）。

Usage:
python .\lstmkf_train.py ^
  --csv "D:\张正琦\tum ei\fp\dataset\ngsim_processed_samples.csv" ^
  --stats-json "D:\张正琦\tum ei\fp\dataset\ngsim_stats.json" ^
  --hist-len 25 --future-len 25 ^
  --batch-size 64 --epochs 2 ^
  --split-by vehicle --val-split 0.1 --test-split 0.1 ^
  --save-splits ^
  --device auto --out ".\outputs\lstmkf_euclid"
...
# 建议先到仓库根并激活环境
# 1) 固定路径变量
REPO="/sq/home/ge23lac/lstm-kf"
SCRIPT="$REPO/lstm-kf/lstmkf-train-test.py"   # 统一用这个
OUT="$REPO/outputs/lstmkf_64000"
LOG="$OUT/train.log"
mkdir -p "$OUT"
...
nohup "$PY" "$REPO/lstm-kf/lstmkf-train-test.py" \
  --csv "$REPO/data/ngsim_processed_samples_64000.csv" \
  --stats-json "$REPO/data/ngsim_stats.json" \
  --hist-len 25 --future-len 25 \
  --batch-size 64 --epochs 20 \
  --lr 3e-4 --weight-decay 1e-4 --dropout 0.2 --grad-clip 1.0 \
  --f-hidden 64 --q-hidden 64 --r-hidden 32 --f-layers 2 \
  --workers 8 --prefetch-factor 4 --amp --device cuda \
  --out "$OUT" \
  --split-by vehicle --val-split 0.1 --test-split 0.1 \
  --save-splits --seed 42 \
  > "$LOG" 2>&1 &
...
打开 TensorBoard:
tensorboard --logdir .\outputs\lstmkf_euclid\runs

在 Windows 查看图:
Start-Process ".\outputs\lstmkf_euclid\final_plot.png"
(如果启用测试评估，还会有 final_plot_test.png)
"""
import os
import json
import argparse
...
class LSTMKF(nn.Module):
    """Kalman 反向传播展开（按可见维度子选择进行更新）。"""
    def __init__(self, f_net, q_net, r_net, fdiag_net=None, jitter=1e-6):
        super().__init__()
        self.f_net = f_net
        self.q_net = q_net
        self.r_net = r_net
    ...
    def forward(self, x_hist_xy, z_seq_xy, z_mask_full21=None, P0=None):
        ...
        q_diag  = self.q_net(x_prior)     # (B,T,14)
        r_diag  = self.r_net(z_seq_xy)    # (B,T,14)
        ...
        P_prev = I_full.clone() if P0 is None else P0.clone()
        ...
        for t in range(T):
            ...
            obs_mask_bt = mask14[:, t, :] > 0.5
            m_counts    = obs_mask_bt.sum(dim=1)
            ...
            for m in unique_m.tolist():
                ...
                P_prior_g = P_pr_t[sel]
                xp_g      = xp_t[sel]
                z_g       = z_seq_xy[sel, t, :]
                r_g       = r_diag[sel, t, :]
                mask_g    = obs_mask_bt[sel]

                B_g = P_prior_g.size(0)
                idx_mat = torch.stack([torch.where(mask_g[i])[0] for i in range(B_g)], dim=0)  # (B_g, m)

                P_ho  = torch.gather(P_prior_g, 2, idx_mat.unsqueeze(1).expand(B_g, D, m))
                Prows = torch.gather(P_prior_g, 1, idx_mat.unsqueeze(-1).expand(B_g, m, D))
                ...
                innov = z_obs - x_obs
                S = P_oo + R_obs + self.jitter * torch.eye(m, device=device, dtype=dtype).unsqueeze(0)
                X = torch.linalg.solve(S, P_ho.transpose(1, 2))
                K = X.transpose(1, 2)  # (B_g,14,m)
...
# Loss & helpers
# ---------------------------
def build_mask14(mask21):
    mask7  = mask21[..., 2::3].float()               # (B,T,7)
    mask14 = torch.repeat_interleave(mask7, 2, dim=-1)
    return mask14, mask7

def masked_time_avg_mse(pred, gt, mask14):
    """(1/T) * sum_t || (gt_t - pred_t) ⊙ mask ||^2 ，再对 batch 平均。"""
    diff = (pred - gt) * mask14              # (B,T,14)
    per_t = (diff ** 2).sum(dim=2)           # (B,T)
    loss_bt = per_t.mean(dim=1)              # (B,)
    return loss_bt.mean()
...
def build_model(args, device):
    ...
    fdiag_net = LSTM_F_Diag(input_dim=14, hidden=args.fdiag_hidden, layers=1, state_dim=14, alpha=0.3, dropout=args.dropout)
    model = LSTMKF(f_net, q_net, r_net, fdiag_net=fdiag_net)
...
def train_one_epoch(model, loader, optimizer, device, scaler, args, writer=None, epoch_idx=0):
    ...
    for x_hist_xy, y_fut_xy, y_fut_full21, x_hist_full21, vids, starts in loader:
        ...
        with torch.cuda.amp.autocast(enabled=(device.type == "cuda" and args.amp)):
            ...
            loss_post  = masked_time_avg_mse(out["x_post"],  y_fut_xy, mask14)
            loss_prior = masked_time_avg_mse(out["x_prior"], y_fut_xy, mask14)
    ...
    if writer: writer.add_scalar("train/loss", avg, epoch_idx)
    return avg
...
def evaluate_epoch(model, loader, device, args, writer=None, epoch_idx=0, tag_prefix="val"):
    ...
    avg = total_loss / max(n_samples, 1)
    if writer: writer.add_scalar(f"{tag_prefix}/loss", avg, epoch_idx)
    return avg
...
def main():
    ...
    p.add_argument("--r-hidden", type=int, default=64)
    ...
    train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True,  num_workers=args.workers, drop_last=False)
    val_loader   = DataLoader(val_set,   batch_size=args.batch_size, shuffle=False, num_workers=args.workers, drop_last=False)
    ...
    if args.resume and os.path.isfile(args.resume):
        ...
        if isinstance(sd, dict) and "state_dict" not in sd and "model_state" not in sd:
            ...
            model.load_state_dict(sd if isinstance(sd, dict) and "state_dict" not in sd else sd.get("model_state", sd))
    ...
    try:
        ...
        o1  = {k: v[idx:idx+1].cpu() for k, v in out.items()}
