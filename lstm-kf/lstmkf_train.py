...
# lstmkf_train_stable.py
# =========================================================
# 稳定版 LSTM-KF 训练脚本（含数值稳健的卡尔曼更新、AMP 新 API）
# - 关键改动：
#   1) 用 _solve_spd_safe 稳健求解 S X = B（对称化 + 自适应对角加载 + Cholesky + pinv 回退）
#   2) R/Q 下界（--min-r/--min-q）与 --jitter 控制，避免奇异
#   3) AMP 使用 torch.amp 新接口（GradScaler/ autocast）
# =========================================================
...
def plot_anchor_scene(
    ...
    if smooth_win and smooth_win > 1:
        ...
        for j in range(N):
            ...
            zf_s_sm[:, j, :] = _smooth_segments(zf_s[:, j, :],  mask=m_fut,  jump_break_ft=jump_break_ft, win=smooth_win)
